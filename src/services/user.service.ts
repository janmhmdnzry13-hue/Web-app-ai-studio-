...
export class UserService extends BaseService implements IUserService {
  ...
  async updateProfile(userId: string, updates: Partial<Profile>): Promise<ServiceResult<User>> {
    try {
      ...
      // If backend explicitly rejected authentication, do not fake update
      if (res.error && (res.error.code === 'UNAUTHORIZED' || res.error.code === 'INVALID_TOKEN' || res.error.code === 'USER_NOT_FOUND')) {
        return this.failure(res.error.code, res.error.message || 'Authentication required to update profile.');
      }

      return this.failure(res.error?.code || 'USER_UPDATE_FAILED', res.error?.message || 'Failed to update profile.');
    } catch (err: any) {
  ...
  async updatePreferences(userId: string, preferences: Partial<UserPreferences>): Promise<ServiceResult<UserPreferences>> {
    try {
      ...
      // If backend explicitly rejected authentication, do not fake update
      if (res.error && (res.error.code === 'UNAUTHORIZED' || res.error.code === 'INVALID_TOKEN' || res.error.code === 'USER_NOT_FOUND')) {
        return this.failure(res.error.code, res.error.message || 'Authentication required to update preferences.');
      }

      return this.failure(res.error?.code || 'USER_PREFS_FAILED', res.error?.message || 'Failed to update preferences.');
    } catch (err: any) {
  ...
  async exportFullUserData(userId: string): Promise<ServiceResult<UserExportArchive>> {
    try {
      ...
      // Backend auth rejection must fail immediately when a token was presented, but fall back to local export when running locally
      if (res.error && (res.error.code === 'INVALID_TOKEN' || res.error.code === 'TOKEN_INVALID' || res.error.code === 'TOKEN_EXPIRED' || res.error.code === 'USER_NOT_FOUND')) {
        return this.failure(res.error.code, res.error.message || 'Authentication required to export data.');
      }

      // If backend network error or offline fallback, compile local client export
      const session = safeStorage.get<AuthSession | null>(APP_CONSTANTS.STORAGE_KEYS.USER_SESSION, null);
      ...
      const archive: UserExportArchive = {
        ...
        user: session?.user || {
          id: userId,
          email: 'user@origin.internal',
          role: 'member',
          emailVerified: true,
          lastLoginAt: new Date().toISOString(),
          createdAt: new Date().toISOString(),
          updatedAt: new Date().toISOString(),
          profile: { displayName: 'User', headline: '', bio: '', primaryLifeFocus: '' },
          preferences: { theme: 'system', timezone: 'UTC', locale: 'en-US', weekStartDay: 1, reducedMotion: false, compactDensity: false, dailyReflectionReminderTime: null, notificationChannels: { inApp: true, email: false, dailyDigest: false } },
        },
        data: {
          tasks,
          goals,
      ...
      return this.failure(res.error?.code || 'EXPORT_FAILED', res.error?.message || 'Failed to export user data.');
    } catch (err: any) {
  ...
  async deleteAccount(userId: string): Promise<ServiceResult<boolean>> {
    try {
      const res = await apiClient.delete('/api/auth/delete-account');
      if (res.success) {
      ...
      if (res.error && (res.error.code === 'INVALID_TOKEN' || res.error.code === 'TOKEN_INVALID' || res.error.code === 'TOKEN_EXPIRED' || res.error.code === 'USER_NOT_FOUND')) {
        return this.failure(res.error.code, res.error.message || 'Authentication required to delete account.');
      }

      // Local / offline purge fallback
      safeStorage.remove(`${APP_CONSTANTS.STORAGE_KEYS.TASKS_PREFIX}${userId}`);
      safeStorage.remove(`${APP_CONSTANTS.STORAGE_KEYS.GOALS_PREFIX}${userId}`);
      safeStorage.remove(`${APP_CONSTANTS.STORAGE_KEYS.HABITS_PREFIX}${userId}`);
      safeStorage.remove(`${APP_CONSTANTS.STORAGE_KEYS.HABIT_LOGS_PREFIX}${userId}`);
